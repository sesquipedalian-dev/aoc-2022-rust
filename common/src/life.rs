...
pub struct Assigner<DimensionType>
where
...
impl<DimensionType> Assigner<DimensionType>
where
    DimensionType: std::convert::TryInto<usize>,
    Coord<DimensionType>: Eq + std::hash::Hash,
{
    pub fn new() -> Self {
        Assigner::<DimensionType> {
            spots: HashMap::new(),
        }
    }
    ...
    pub fn assign(&mut self, row: isize, column: isize, value: LifeOption) {
        self.spots
            .insert(Coord::<DimensionType>::new_2d(row, column), value);
    }

    pub fn assign_3d(&mut self, row: isize, column: isize, z: isize, value: LifeOption) {
        self.spots
            .insert(Coord::<DimensionType>::new_3d(row, column, z), value);
    }
...
pub struct Coord<DimensionType>
where
    DimensionType: std::convert::TryInto<usize>,
{
    pub dim: Vec<isize>,
...
impl<DimensionType> Coord<DimensionType>
where
    DimensionType: std::convert::TryInto<usize>,
{
    ...
    pub fn set(
        ...
        value: isize,
    ) -> Result<(), DimensionType::Error> {
        Ok(self.dim[dimension.try_into()?] = value)
    }
}

impl<DimensionType> Clone for Coord<DimensionType>
where
    DimensionType: std::convert::TryInto<usize>,
{
    fn clone(&self) -> Self {
...
pub struct LifeSpace<DimensionType>
where
    DimensionType: std::convert::TryInto<usize>,
{
    pub spots: HashMap<Coord<DimensionType>, LifeOption>,
...
impl<DimensionType> LifeSpace<DimensionType>
where
    DimensionType: std::convert::TryInto<usize>,
    ...
    Coord<DimensionType>: Eq + std::hash::Hash,
{
    pub fn new(input: &[String], dimensions: usize) -> Result<Self, Error> {
        let mut row = 0;
        let mut result = LifeSpace::<DimensionType> {
            spots: HashMap::new(),
            tentative_spots: HashMap::new(),
        };
    ...
    pub fn neighbors_skip_floor(
        ...
        NeighborIterator::<DimensionType> {
            ...
            column,
            count: 0,
            skip_chars: Some(LifeOption::Floor),
            direction_count: 1,
        }
    }
    ...
    pub fn to_string_rec(
        &self,
        min_maxes: &Vec<(isize, isize)>,
        dim: usize,
        prev_dim_coords: &VecDeque<isize>,
        accum: &mut String,
    ) -> Result<(), Error> {
        for i in min_maxes[dim].0..=min_maxes[dim].1 {
            ...
            if dim == 0 {
                ...
                let new_coord = this_coord
                    .iter()
                    .take(2)
                    .rev()
                    .chain(this_coord.iter().skip(2));
                let this_coord = Coord::<DimensionType> {
                    dim: new_coord.map(|s| *s).collect(),
                    pd: PhantomData,
                };
...
pub struct NeighborIterator<'a, DimensionType>
where
    DimensionType: std::convert::TryInto<usize>,
    DimensionType: std::convert::TryFrom<usize>,
{
...
impl<DimensionType> NeighborIterator<'_, DimensionType>
where
    DimensionType: std::convert::TryInto<usize>,
    DimensionType: std::convert::TryFrom<usize>,
{
...
impl Iterator for NeighborIterator<'_, TwoD> {
    ...
    fn next(&mut self) -> Option<LifeOption> {
        ...
        let value = self
            .spots
            .get(&Coord::<TwoD>::new_2d(row, column))
            .map(|s| *s);
...
mod tests {
    ...
    fn test_to_string_3d() {
        ...
        ls.spots
            .insert(Coord::<ThreeD>::new_3d(0, 0, 1), LifeOption::Occupied);
    ...
    fn test_neighbors_limits() {
        ...
        assert_eq!(
            result,
            vec!(LifeOption::Unoccupied, LifeOption::Floor, LifeOption::Floor,)
        );
    }
    ...
    fn test_neighbors_skip_floor_one() {
        ...
        let result: Vec<LifeOption> = iter.collect();
        assert_eq!(result, vec!(LifeOption::Unoccupied,));
    }
